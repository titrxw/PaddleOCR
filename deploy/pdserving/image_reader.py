...
class DBPostProcess(object):
    ...
    def __call__(self, outs_dict, ratio_list):
        ...
        for batch_index in range(pred.shape[0]):
            ...
            class_scores = []
            if classes is None:
                tmp_boxes, tmp_scores = self.boxes_from_bitmap(pred[batch_index], segmentation[batch_index], None, width, height)
            else:
                tmp_boxes, tmp_scores, class_indexes, class_scores = self.boxes_from_bitmap(pred[batch_index], segmentation[batch_index],classes[batch_index],width, height)
